...
save(long,file=paste(outdir,"/M1elong",iter,".RData",sep=""))


## Define model


modelfile <- "~/bipartitemodelsBC/finalmodels/jagsNB/betaT/m1e.txt"      
...
save(output, file = paste(outdir,"/M1eoutput",iter,".RData",sep=""))
...
sink(file=paste(outdir,"/M1eprintoutput",iter,".txt",sep=""))
