...
save(long,file=paste(outdir,"/M1along",iter,".RData",sep=""))

## Define model


modelfile <- "~/bipartitemodelsBC/finalmodels/jagsNB/betaT/M1a.txt"    
...
save(output, file = paste(outdir,"/M1aoutput",iter,".RData",sep=""))
...
sink(file=paste(outdir,"/M1aprintoutput",iter,".txt",sep=""))
