...
save(long,file=paste(outdir,"/M1hlong",iter,".RData",sep=""))

## Define model


modelfile <- "~/bipartitemodelsBC/finalmodels/jagsNB/betaT/m1h.txt"    
...
save(output, file = paste(outdir,"/M1houtput",iter,".RData",sep=""))
...
sink(file=paste(outdir,"/M1hprintoutput",iter,".txt",sep=""))
