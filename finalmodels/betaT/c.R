...
save(long,file=paste(outdir,"/M1clong",iter,".RData",sep=""))

## Define model


modelfile <- "~/bipartitemodelsBC/finalmodels/jagsNB/betaT/m1c.txt" 
...
save(output, file = paste(outdir,"/M1coutput",iter,".RData",sep=""))
...
sink(file=paste(outdir,"/M1cprintoutput",iter,".txt",sep=""))
