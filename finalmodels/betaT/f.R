...
nind <- length(unique(collec.lng$ID))
collec.lng$ID <- NULL
...
long$par.fam <- rep(covars$Superfamily,each=nind)
long$Npar.fam <- length(unique(long$par.fam))

save(long,file=paste(outdir,"/M1flong",iter,".RData",sep=""))

## Define model


modelfile <- "~/bipartitemodelsBC/finalmodels/jagsNB/betaT/m1f.txt"    
...
inits <- function() {
  list(
    ...
    beta=rnorm(long$Npar.fam),
    use=matrix(rep(0.9,long$Npar*long$Nhost.sp),ncol=long$Npar,byrow=T)
  )
}

## Run model
output  <- jags(long, inits = inits, c('mn', 'sd', 'use','phi','mu'), modelfile, n.chains=3, n.iter=iter,n.burnin=burnin,n.thin=thin) # or use defaults

save(output, file = paste(outdir,"/M1foutput",iter,".RData",sep=""))
...
sink(file=paste(outdir,"/M1fprintoutput",iter,".txt",sep=""))
