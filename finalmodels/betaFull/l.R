...
save(long,file=paste(outdir,"/M1llong",iter,".RData",sep=""))

## Define model


modelfile <- "~/bipartitemodelsBC/finalmodels/jagsNB/betaFull/m1l.txt"   
...
## Run model
output  <- jags(long, inits = inits, c('mn', 'sd', 'use','beta','alpha','r','HB_invert','PD_host','hosts','phi','beta_pr'), modelfile, n.chains=3, n.iter=iter,n.burnin=burnin,n.thin=thin) # or use defaults
...
sink(file=paste(outdir,"/M1lprintoutput",iter,".txt",sep=""))
