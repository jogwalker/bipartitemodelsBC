...
save(long,file=paste(outdir,"/M1jlong",iter,".RData",sep=""))

## Define model


modelfile <- "~/bipartitemodelsBC/finalmodels/jagsNB/betaFull/m1j.txt"  
...
save(output, file = paste(outdir,"/M1joutput",iter,".RData",sep=""))
...
sink(file=paste(outdir,"/M1jprintoutput",iter,".txt",sep=""))
