...
save(long,file=paste(outdir,"/M1ilong",iter,".RData",sep=""))
## Define model


modelfile <- "~/bipartitemodelsBC/finalmodels/jagsNB/betaFull/m1i.txt"     
...
## Run model
output  <- jags(long, inits = inits, c('mn', 'sd', 'use', 'phi','beta','alpha','r','HB_invert','PD_host','hosts','alpha_cov'), modelfile, n.chains=3, n.iter=iter,n.burnin=burnin,n.thin=thin) # or use defaults

save(output, file = paste(outdir,"/M1ioutput",iter,".RData",sep=""))
...
sink(file=paste(outdir,"/M1iprintoutput",iter,".txt",sep=""))
