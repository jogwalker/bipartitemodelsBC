...
save(long,file=paste(outdir,"/cmlong.RData",sep=""))
...
{    
  ...
  { 
    ...
    log(lambda[i]) <- beta[host.sp[i],par.sp[i]]   + beta_temp*temp[ind[i]]   # characteristic abundance of inv.sp. j on pl.ind. i (NOTE: insect has the same abundance on all plant species)
  }   
...
} '    , file=(modelfile <- "~/bipartitemodelsBC/finalmodels/jags/phase4/cm.txt"))
...
## Run model
output  <- jags(long, inits = inits, c('mn', 'sd', 'use', 'HB_invert', 'PD_host', 'beta','beta_temp', 'alpha','prec.beta'), modelfile, n.chains=3, n.iter=iter) # or use defaults

save(output, file = paste(outdir,"/cm_output.RData",sep=""))
...
sink(file=paste(outdir,"/cm_printoutput.txt",sep=""))
