...
save(long,file=paste(outdir,"/elxnlong.RData",sep=""))
...
} '    , file=(modelfile <- "~/bipartitemodelsBC/finalmodels/jags/phase4/elxn.txt"))   
...
save(output, file = paste(outdir,"/elxn_output.RData",sep=""))
...
sink(file=paste(outdir,"/elxn_printoutput.txt",sep=""))
