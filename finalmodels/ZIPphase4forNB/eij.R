...
save(long,file=paste(outdir,"/eijlong.RData",sep=""))
...
} '    , file=(modelfile <- "~/bipartitemodelsBC/finalmodels/jags/phase4/eij.txt"))     
...
## Run model
output  <- jags(long, inits = inits, c('mn', 'sd', 'use', 'HB_invert', 'PD_host', 'beta', 'alpha','alpha_r','alpha_d'), modelfile, n.chains=3, n.iter=iter) # or use defaults

save(output, file = paste(outdir,"/eij_output.RData",sep=""))
...
sink(file=paste(outdir,"/eij_printoutput.txt",sep=""))
