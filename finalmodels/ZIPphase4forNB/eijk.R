...
save(long,file=paste(outdir,"/eijklong.RData",sep=""))
...
{    
  ...
  { 
    ...
    log(lambda[i]) <- beta[par.gen[i]]      # characteristic abundance of inv.sp. j on pl.ind. i (NOTE: insect has the same abundance on all plant species)
  }   
...
} '    , file=(modelfile <- "~/bipartitemodelsBC/finalmodels/jags/phase4/eijk.txt"))     
...
## Run model
output  <- jags(long, inits = inits, c('mn', 'sd', 'use', 'HB_invert', 'PD_host', 'beta', 'alpha','alpha_r','alpha_d','alpha_b','alpha_m'), modelfile, n.chains=3, n.iter=iter) # or use defaults

save(output, file = paste(outdir,"/eijk_output.RData",sep=""))
...
sink(file=paste(outdir,"/eijk_printoutput.txt",sep=""))
