...
save(long,file=paste(outdir,"/cijlong.RData",sep=""))
...
{    
  ...
  { 
    ...
    log(lambda[i]) <- beta[host.sp[i],par.sp[i]]      # characteristic abundance of inv.sp. j on pl.ind. i (NOTE: insect has the same abundance on all plant species)
  }   
  ...
  for (n in 1: Nhost.sp) #loop around all plant species, calculate use, alpha and insect diversity
  {
    ...
    {  
      ...
      logit(p[n, j]) <- alpha[n, j] + alpha_r*ruminant[n] + alpha_d*domestic[n]  # p is pi, logit is link function of Bernoulli distribution
...
} '    , file=(modelfile <- "~/bipartitemodelsBC/finalmodels/jags/phase4/cij.txt"))     
...
## Run model
output  <- jags(long, inits = inits, c('mn', 'sd', 'use', 'HB_invert', 'PD_host', 'beta', 'alpha','alpha_r','alpha_d','prec_beta'), modelfile, n.chains=3, n.iter=iter) # or use defaults

save(output, file = paste(outdir,"/cij_output.RData",sep=""))
...
sink(file=paste(outdir,"/cij_printoutput.txt",sep=""))
