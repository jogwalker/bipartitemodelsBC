...
save(long,file=paste(outdir,"/clxnlong.RData",sep=""))
...
{    
  ...
  { 
    ...
    log(lambda[i]) <- beta[host.sp[i],par.sp[i]]   + beta_pr[1]*rain[ind[i]] + beta_pr[2]*rain[ind[i]]*treated[ind[i]]+ beta_t*treated[ind[i]]   # characteristic abundance of inv.sp. j on pl.ind. i (NOTE: insect has the same abundance on all plant species)
  }   
...
} '    , file=(modelfile <- "~/bipartitemodelsBC/finalmodels/jags/phase4/clxn.txt"))   
...
## Run model
output  <- jags(long, inits = inits, c('mn', 'sd', 'use', 'HB_invert', 'PD_host', 'beta','beta_pr','alpha','beta_t','prec.beta'), modelfile, n.chains=3, n.iter=iter) # or use defaults

save(output, file = paste(outdir,"/clxn_output.RData",sep=""))
...
sink(file=paste(outdir,"/clxn_printoutput.txt",sep=""))
