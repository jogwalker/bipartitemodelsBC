...
save(long,file=paste(outdir,"/lnlong.RData",sep=""))
...
{    
  ...
  { 
    ...
    log(lambda[i]) <- beta[par.sp[i]]   + beta_pr*rain[ind[i]] + beta_t*treated[ind[i]]  # characteristic abundance of inv.sp. j on pl.ind. i (NOTE: insect has the same abundance on all plant species)
  }   
...
} '    , file=(modelfile <- "~/bipartitemodelsBC/finalmodels/jags/ln.txt"))   
...
save(output, file = paste(outdir,"/ln_output.RData",sep=""))
...
sink(file=paste(outdir,"/ln_printoutput.txt",sep=""))
