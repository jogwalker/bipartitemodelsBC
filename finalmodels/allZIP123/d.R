...
save(long,file=paste(outdir,"/M1dlong.RData",sep=""))
...
{    
  ...
  for (n in 1: Nhost.sp) #loop around all plant species, calculate use, alpha and insect diversity
  {
    ...
    {  
      ...
      beta[n,j] ~ dnorm(sp.beta[j],prec.beta[j])
...
} '    , file=(modelfile <- "~/bipartitemodelsBC/finalmodels/jags/m1d.txt"))    
...
save(output, file = paste(outdir,"/M1doutput.RData",sep=""))
...
sink(file=paste(outdir,"/M1dprintoutput.txt",sep=""))
