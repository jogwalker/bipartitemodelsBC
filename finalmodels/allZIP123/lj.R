...
save(long,file=paste(outdir,"/ljlong.RData",sep=""))
...
} '    , file=(modelfile <- "~/bipartitemodelsBC/finalmodels/jags/lj.txt"))   
...
## Run model
output  <- jags(long, inits = inits, c('mn', 'sd', 'use', 'HB_invert', 'PD_host', 'beta','beta_pr','alpha','alpha_cov'), modelfile, n.chains=3, n.iter=iter) # or use defaults

save(output, file = paste(outdir,"/lj_output.RData",sep=""))
...
sink(file=paste(outdir,"/lj_printoutput.txt",sep=""))
