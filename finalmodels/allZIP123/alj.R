...
long$hostspchar <- as.numeric(covars.host$Domestic)

save(long,file=paste(outdir,"/aljlong.RData",sep=""))
...
} '    , file=(modelfile <- "~/bipartitemodelsBC/finalmodels/jags/alj.txt"))    
...
save(output, file = paste(outdir,"/alj_output.RData",sep=""))
...
sink(file=paste(outdir,"/alj_printoutput.txt",sep=""))
