...
long$hostspchar <- as.numeric(covars.host$Ruminant)

save(long,file=paste(outdir,"/alilong.RData",sep=""))
...
cat('      model 
  ...
  for (i in 1:Nobs) { #for all observations run loop around 
    ...
    log(lambda[i]) <- beta[par.sp[i]]  + beta_pr*rain[ind[i]] # mean abundance of inv.sp. j on pl.ind. i
  }  
  ...
  for (n in 1: Nhost.sp) { #loop around all host species, calculate use, alpha and insect diversity
    ...
    for (j in 1:Npar) {  
      ...
      logit(p[n, j]) <- alpha[n, par.gen[j]] + alpha_cov*hostspchar[n]# refers to the identity list which names the category for each 
    }
...
} '    , file=(modelfile <- "~/bipartitemodelsBC/finalmodels/jags/ali.txt"))    
...
## Run model
output  <- jags(long, inits = inits, c('mn', 'sd', 'use', 'HB_invert', 'PD_host', 'beta', 'alpha','beta_pr','alpha_cov'), modelfile, n.chains=3, n.iter=iter) # or use defaults

save(output, file = paste(outdir,"/ali_output.RData",sep=""))
...
sink(file=paste(outdir,"/ali_printoutput.txt",sep=""))
