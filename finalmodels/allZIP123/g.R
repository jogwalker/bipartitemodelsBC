...
save(long,file=paste(outdir,"/M1glong.RData",sep=""))
...
{    
  ...
  for (n in 1: Nhost.sp) #loop around all plant species, calculate use, alpha and insect diversity
  {
    ...
    {  
      ...
      logit(p[n, j]) <- alpha[n, j] + alpha_gen[hostgen[n]]# p is pi, logit is link function of Bernoulli distribution
...
} '    , file=(modelfile <- "~/bipartitemodelsBC/finalmodels/jags/m1g.txt")) 
...
save(output, file = paste(outdir,"/M1goutput.RData",sep=""))
...
sink(file=paste(outdir,"/M1gprintoutput.txt",sep=""))
