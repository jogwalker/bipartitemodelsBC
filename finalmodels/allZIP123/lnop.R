...
save(long,file=paste(outdir,"/lnoplong.RData",sep=""))
...
} '    , file=(modelfile <- "~/bipartitemodelsBC/finalmodels/jags/lnop.txt"))   
...
## Run model
output  <- jags(long, inits = inits, c('mn', 'sd', 'use', 'HB_invert', 'PD_host', 'beta','beta_pr','alpha','beta_t','beta_s','beta_j'), modelfile, n.chains=3, n.iter=iter) # or use defaults

save(output, file = paste(outdir,"/lnop_output.RData",sep=""))
...
sink(file=paste(outdir,"/lnop_printoutput.txt",sep=""))
