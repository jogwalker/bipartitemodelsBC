...
save(long,file=paste(outdir,"/M1mlong.RData",sep=""))
...
{    
  ...
  { 
    ...
    log(lambda[i]) <- beta[par.sp[i]]   + beta_temp*temp[ind[i]]   # characteristic abundance of inv.sp. j on pl.ind. i (NOTE: insect has the same abundance on all plant species)
  }   
...
} '    , file=(modelfile <- "~/bipartitemodelsBC/finalmodels/jags/m1m.txt"))
...
save(output, file = paste(outdir,"/M1moutput.RData",sep=""))
...
sink(file=paste(outdir,"/M1mprintoutput.txt",sep=""))
