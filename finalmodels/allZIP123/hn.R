...
{    
  ...
  { 
    ...
    log(lambda[i]) <- beta[par.sp[i]] + beta_t*treated[ind[i]]      # characteristic abundance of inv.sp. j on pl.ind. i (NOTE: insect has the same abundance on all plant species)
  }   
...
} '    , file=(modelfile <- "~/bipartitemodelsBC/finalmodels/jags/hn.txt"))    
