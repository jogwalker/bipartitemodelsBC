...
save(long,file=paste(outdir,"/M1nlongQUICK.RData",sep=""))
...
## Run model
output  <- jags(long, inits = inits, c('mn', 'sd', 'beta_t'), modelfile, n.chains=3, n.iter=iter, n.burnin=5000,n.thin=100) # or use defaults

save(output, file = paste(outdir,"/M1noutputQUICK.RData",sep=""))
...
sink(file=paste(outdir,"/M1nprintoutputQUICK.txt",sep=""))
