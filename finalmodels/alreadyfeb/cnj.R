...
save(long,file=paste(outdir,"/cnjlongQUICK.RData",sep=""))
...
## Run model
output  <- jags(long, inits = inits, c('mn', 'sd', 'alpha_d','beta_t'), modelfile, n.chains=3, n.iter=iter,n.burnin=5000,n.thin=100) # or use defaults

save(output, file = paste(outdir,"/cnj_outputQUICK.RData",sep=""))
...
sink(file=paste(outdir,"/cnj_printoutputQUICK.txt",sep=""))
