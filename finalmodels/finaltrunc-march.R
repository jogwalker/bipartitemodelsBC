...
save(long,file=paste(outdir,"/march-cnjlong-trunc",iter,".RData",sep=""))

## Define model


modelfile <- "~/bipartitemodelsBC/finalmodels/jagsNB/finaltrunc/finaltrunc.txt"     
...
## Run model
output  <- jags(long, inits = inits, c('mn', 'sd', 'use', 'HB_invert', 'PD_host', 'beta', 'alpha','alpha_d','prec.beta','r','beta_t','hosts','parasites'), modelfile, n.chains=3, n.iter=iter,n.burnin=burn,n.thin=thin) # or use defaults

save(output, file = paste(outdir,"/march-cnj_output-trunc",iter,".RData",sep=""))
...
sink(file=paste(outdir,"/march-cnj_printoutput-trunc",iter,".txt",sep=""))
