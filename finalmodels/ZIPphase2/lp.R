...
missing.fem <- which(is.na(covars$Sex))
...
long$female <- female
long$missing.ind.fem <- missing.fem

save(long,file=paste(outdir,"/lplong.RData",sep=""))
...
{    
  ...
  { 
    ...
    log(lambda[i]) <- beta[par.sp[i]]   + beta_pr*rain[ind[i]] + beta_s*female[ind[i]]  # characteristic abundance of inv.sp. j on pl.ind. i (NOTE: insect has the same abundance on all plant species)
  }   
...
} '    , file=(modelfile <- "~/bipartitemodelsBC/finalmodels/jags/lp.txt"))   
...
## Run model
output  <- jags(long, inits = inits, c('mn', 'sd', 'use', 'HB_invert', 'PD_host', 'beta','beta_pr','alpha','beta_s'), modelfile, n.chains=3, n.iter=iter) # or use defaults

save(output, file = paste(outdir,"/lp_output.RData",sep=""))
...
sink(file=paste(outdir,"/lp_printoutput.txt",sep=""))
