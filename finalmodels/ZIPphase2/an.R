...
save(long,file=paste(outdir,"/anlong.RData",sep=""))
...
cat('      model 
  ...
  for (i in 1:Nobs) { #for all observations run loop around 
    ...
    log(lambda[i]) <- beta[par.sp[i]] + beta_t*treated[ind[i]]# mean abundance of inv.sp. j on pl.ind. i
  }  
...
} '    , file=(modelfile <- "~/bipartitemodelsBC/finalmodels/jags/an.txt"))    
...
save(output, file = paste(outdir,"/an_output.RData",sep=""))
...
sink(file=paste(outdir,"/an_printoutput.txt",sep=""))
