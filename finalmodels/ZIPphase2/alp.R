...
missing.fem <- which(is.na(covars$Sex)) # 35% missing
...
save(long,file=paste(outdir,"/alplong.RData",sep=""))
...
} '    , file=(modelfile <- "~/bipartitemodelsBC/finalmodels/jags/alp.txt"))    
...
## Run model
output  <- jags(long, inits = inits, c('mn', 'sd', 'use', 'HB_invert', 'PD_host', 'beta', 'alpha','beta_pr','beta_s'), modelfile, n.chains=3, n.iter=iter) # or use defaults

save(output, file = paste(outdir,"/alp_output.RData",sep=""))
...
sink(file=paste(outdir,"/alp_printoutput.txt",sep=""))
