...
save(long,file=paste(outdir,"/hklong.RData",sep=""))
...
{    
  ...
  for (n in 1: Nhost.sp) #loop around all plant species, calculate use, alpha and insect diversity
  {
    ...
    {  
      ...
      logit(p[n, j]) <- alpha[hostgen[n], j] + alpha_b*browser[n] + alpha_m*mixed[n] # p is pi, logit is link function of Bernoulli distribution
...
} '    , file=(modelfile <- "~/bipartitemodelsBC/finalmodels/jags/hk.txt"))    
...
## Run model
output  <- jags(long, inits = inits, c('mn', 'sd', 'use', 'HB_invert', 'PD_host', 'beta', 'alpha','alpha_b','alpha_m','browsmixdiff'), modelfile, n.chains=3, n.iter=iter) # or use defaults

save(output, file = paste(outdir,"/hk_output.RData",sep=""))
...
sink(file=paste(outdir,"/hk_printoutput.txt",sep=""))
