...
missing <- which(is.na(covars$Sex))
...
save(long,file=paste(outdir,"/hplong.RData",sep=""))
...
{    
  ...
  { 
    ...
    log(lambda[i]) <- beta[par.sp[i]] + beta_s*female[ind[i]]      # characteristic abundance of inv.sp. j on pl.ind. i (NOTE: insect has the same abundance on all plant species)
  }   
...
} '    , file=(modelfile <- "~/bipartitemodelsBC/finalmodels/jags/hp.txt"))    
...
save(output, file = paste(outdir,"/hp_output.RData",sep=""))
...
sink(file=paste(outdir,"/hp_printoutput.txt",sep=""))
