...
cat('      model 
  ...
  for (i in 1:Nobs) { #for all observations run loop around 
    ...
    log(lambda[i]) <- beta[par.sp[i]]  + beta_pr*rain[ind[i]] + beta_t*treated[ind[i]] # mean abundance of inv.sp. j on pl.ind. i
  }  
...
} '    , file=(modelfile <- "~/bipartitemodelsBC/finalmodels/jags/aln.txt"))    
