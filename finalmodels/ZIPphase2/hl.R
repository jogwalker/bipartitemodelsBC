...
save(long,file=paste(outdir,"/hllong.RData",sep=""))
...
{    
  ...
  for (n in 1: Nhost.sp) #loop around all plant species, calculate use, alpha and insect diversity
  {
    ...
    {  
      ...
      logit(p[n, j]) <- alpha[hostgen[n], j]# p is pi, logit is link function of Bernoulli distribution
       # coefficient for use is drawn from normal distribution with mean and precision)
    }
...
} '    , file=(modelfile <- "~/bipartitemodelsBC/finalmodels/jags/hl.txt"))    
...
## Run model
output  <- jags(long, inits = inits, c('mn', 'sd', 'use', 'HB_invert', 'PD_host', 'beta', 'alpha','beta_pr'), modelfile, n.chains=3, n.iter=iter) # or use defaults

save(output, file = paste(outdir,"/hl_output.RData",sep=""))
...
sink(file=paste(outdir,"/hl_printoutput.txt",sep=""))
