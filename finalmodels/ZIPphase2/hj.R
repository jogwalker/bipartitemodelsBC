...
save(long,file=paste(outdir,"/hjlong.RData",sep=""))
...
{    
  ...
  for (n in 1: Nhost.sp) #loop around all plant species, calculate use, alpha and insect diversity
  {
    ...
    {  
      ...
      logit(p[n, j]) <- alpha[hostgen[n], j] + alpha_cov*hostspchar[n] # p is pi, logit is link function of Bernoulli distribution
...
} '    , file=(modelfile <- "~/bipartitemodelsBC/finalmodels/jags/hj.txt"))    
...
save(output, file = paste(outdir,"/hj_output.RData",sep=""))
...
sink(file=paste(outdir,"/hj_printoutput.txt",sep=""))
