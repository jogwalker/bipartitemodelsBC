...
save(long,file=paste(outdir,"/cllong.RData",sep=""))

## Define model


modelfile <- "~/bipartitemodelsBC/finalmodels/jagsNB/phase5/cl.txt"   
...
## Run model
output  <- jags(long, inits = inits, c('mn', 'sd', 'use', 'HB_invert', 'PD_host', 'beta','beta_pr','alpha','prec.beta','r'), modelfile, n.chains=3, n.iter=iter) # or use defaults

save(output, file = paste(outdir,"/cl_output.RData",sep=""))
...
sink(file=paste(outdir,"/cl_printoutput.txt",sep=""))
