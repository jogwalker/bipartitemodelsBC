...
save(long,file=paste(outdir,"/cklong.RData",sep=""))

## Define model


modelfile <- "~/bipartitemodelsBC/finalmodels/jagsNB/phase5/ck.txt"     
...
## Run model
output  <- jags(long, inits = inits, c('mn', 'sd', 'use', 'HB_invert', 'PD_host', 'beta', 'alpha','alpha_b','alpha_m','prec.beta','r'), modelfile, n.chains=3, n.iter=iter) # or use defaults

save(output, file = paste(outdir,"/ck_output.RData",sep=""))
...
sink(file=paste(outdir,"/ck_printoutput.txt",sep=""))
