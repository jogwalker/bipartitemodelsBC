...
save(long,file=paste(outdir,"/cnlong.RData",sep=""))

## Define model


modelfile <- "~/bipartitemodelsBC/finalmodels/jagsNB/phase5/cn.txt"     
...
## Run model
output  <- jags(long, inits = inits, c('mn', 'sd', 'use', 'HB_invert', 'PD_host', 'beta', 'alpha','prec.beta','r','beta_t'), modelfile, n.chains=3, n.iter=iter) # or use defaults

save(output, file = paste(outdir,"/cn_output.RData",sep=""))
...
sink(file=paste(outdir,"/cn_printoutput.txt",sep=""))
