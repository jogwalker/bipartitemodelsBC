...
save(long,file=paste(outdir,"/cilong.RData",sep=""))

## Define model


modelfile <- "~/bipartitemodelsBC/finalmodels/jagsNB/phase5/ci.txt"     
...
## Run model
output  <- jags(long, inits = inits, c('mn', 'sd', 'use', 'HB_invert', 'PD_host', 'beta', 'alpha','alpha_r','prec.beta','r'), modelfile, n.chains=3, n.iter=iter) # or use defaults

save(output, file = paste(outdir,"/ci_output.RData",sep=""))
...
sink(file=paste(outdir,"/ci_printoutput.txt",sep=""))
