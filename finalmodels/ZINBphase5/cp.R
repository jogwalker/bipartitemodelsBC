...
missing <- which(is.na(covars$Sex)) # 35% missing
...
save(long,file=paste(outdir,"/cplong.RData",sep=""))

## Define model


modelfile <- "~/bipartitemodelsBC/finalmodels/jagsNB/phase5/cp.txt" 
...
## Run model
output  <- jags(long, inits = inits, c('mn', 'sd', 'use', 'HB_invert', 'PD_host', 'beta', 'alpha','r','prec.beta','beta_s'), modelfile, n.chains=3, n.iter=iter) # or use defaults

save(output, file = paste(outdir,"/cp_output.RData",sep=""))
...
sink(file=paste(outdir,"/cp_printoutput.txt",sep=""))
