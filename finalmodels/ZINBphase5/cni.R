...
save(long,file=paste(outdir,"/cnilong.RData",sep=""))

## Define model


modelfile <- "~/bipartitemodelsBC/finalmodels/jagsNB/phase5/cni.txt"     
...
## Run model
output  <- jags(long, inits = inits, c('mn', 'sd', 'use', 'HB_invert', 'PD_host', 'beta', 'alpha','alpha_r','prec.beta','r','beta_t'), modelfile, n.chains=3, n.iter=iter) # or use defaults

save(output, file = paste(outdir,"/cni_output.RData",sep=""))
...
sink(file=paste(outdir,"/cni_printoutput.txt",sep=""))
