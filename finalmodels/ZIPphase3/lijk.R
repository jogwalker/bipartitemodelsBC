...
save(long,file=paste(outdir,"/lijklong.RData",sep=""))
...
{    
  ...
  { 
    ...
    log(lambda[i]) <- beta[par.sp[i]]   + beta_pr*rain[ind[i]]   # characteristic abundance of inv.sp. j on pl.ind. i (NOTE: insect has the same abundance on all plant species)
  }   
  ...
  for (n in 1: Nhost.sp) #loop around all plant species, calculate use, alpha and insect diversity
  {
    ...
    {  
      ...
      logit(p[n, j]) <- alpha[n, j] + alpha_r*ruminant[n] + alpha_d*domestic[n] + alpha_b*browser[n] + alpha_m*mixed[n] # p is pi, logit is link function of Bernoulli distribution
  ...
  for(i in 1:length(missing.ind)) {
    rain[missing.ind[i]] ~ dnorm(0,1)T(-1.738577, )
  }  
  ...
  alpha_r ~ dnorm(0, .0001)
  alpha_d ~ dnorm(0, .0001)
  alpha_b ~ dnorm(0, .0001)
  alpha_m ~ dnorm(0, .0001)
...
} '    , file=(modelfile <- "~/bipartitemodelsBC/finalmodels/jags/lijk.txt"))   
...
## Run model
output  <- jags(long, inits = inits, c('mn', 'sd', 'use', 'HB_invert', 'PD_host', 'beta','beta_pr','alpha','alpha_r','alpha_d','alpha_b','alpha_m'), modelfile, n.chains=3, n.iter=iter) # or use defaults

save(output, file = paste(outdir,"/lijk_output.RData",sep=""))
...
sink(file=paste(outdir,"/lijk_printoutput.txt",sep=""))
