...
save(long,file=paste(outdir,"/noplong.RData",sep=""))
...
{    
  ...
  { 
    ...
    log(lambda[i]) <- beta[par.sp[i]]   + beta_t*treated[ind[i]] + beta_s*female[ind[i]] +  beta_j*juvenile[ind[i]] 
  }   
...
} '    , file=(modelfile <- "~/bipartitemodelsBC/finalmodels/jags/nop.txt"))   
...
## Run model
output  <- jags(long, inits = inits, c('mn', 'sd', 'use', 'HB_invert', 'PD_host', 'beta','alpha','beta_t','beta_s','beta_j'), modelfile, n.chains=3, n.iter=iter) # or use defaults

save(output, file = paste(outdir,"/nop_output.RData",sep=""))
...
sink(file=paste(outdir,"/nop_printoutput.txt",sep=""))
