...
save(long,file=paste(outdir,"/M1plong.RData",sep=""))
...
} '    , file=(modelfile <- "~/bipartitemodelsBC/finalmodels/jags/m1p.txt"))   
...
save(output, file = paste(outdir,"/M1poutput.RData",sep=""))
...
sink(file=paste(outdir,"/M1pprintoutput.txt",sep=""))
