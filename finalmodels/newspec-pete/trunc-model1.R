#### RUN SIMPLEST MODEL (for BC): Model 1
...
save(long,file=paste(outdir,"/tM1long",iter,".RData",sep=""))


## Define model


modelfile <- "~/bipartitemodelsBC/finalmodels/jagsNB/newspec-pete/m1t.txt"    
...
## Run model
output  <- jags(long, inits = inits, c('mn', 'sd', 'use', 'HB_invert', 'PD_host', 'beta', 'alpha','r','prec.beta'), modelfile, n.chains=3, n.iter=iter) # or use defaults

save(output, file = paste(outdir,"/tM1_output",iter,".RData",sep=""))
...
sink(file=paste(outdir,"/tM1printoutput",iter,".txt",sep=""))
