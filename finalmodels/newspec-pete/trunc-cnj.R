...
save(long,file=paste(outdir,"/t-cnjlong",iter,".RData",sep=""))


## Define model


modelfile <- "~/bipartitemodelsBC/finalmodels/jagsNB/newspec-pete/cnjt.txt"     
...
## Run model
output  <- jags(long, inits = inits, c('mn', 'sd', 'use', 'HB_invert', 'PD_host', 'beta', 'alpha','alpha_d','prec.beta','r','beta_t'), modelfile, n.chains=3, n.iter=iter) # or use defaults

save(output, file = paste(outdir,"/tcnj_output",iter,".RData",sep=""))
...
sink(file=paste(outdir,"/tcnj_printoutput",iter,".txt",sep=""))
