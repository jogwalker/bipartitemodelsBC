...
#load data 
load("~/dat/bipartitemodelsBC/thesis revision/cnj_output-trunc1e+05.RData")

# don't need to remake fig 3.2
# pdf("~/dat/bipartitemodelsBC/thesis revision/trace-300k-long.pdf",height=10)
# traceplot(output,mfrow=c(4,2),varname=c('mn', 'sd','alpha_d','beta_t','r','deviance'),ask=FALSE)
# dev.off()
attach.jags(output)
### make 2d histogram to demonstrate correlation between mn[1] and sd[1]

pdf("~/dat/bipartitemodelsBC/thesis revision/sdmn2d.pdf")
qplot(mn[,1],sd[,1], geom='bin2d') + labs(y=expression(paste("Standard deviation of ",alpha[1], " (sd[1])")),x=expression(paste("Mean of ",alpha[1], " (mn[1])")) )+ theme_bw()
dev.off()

# don't need to remake fig 3.3
...
load("~/dat/bipartitemodelsBC/thesis revision/cnjlong-trunc1e+05.RData")
...
library(ggplot2)
pdf("~/dat/bipartitemodelsBC/thesis revision/probuse.pdf",height=4)
...
pdf("~/dat/bipartitemodelsBC/thesis revision/abund.pdf",height=4)
...
# useabund2$lowuse <- ifelse(useabund2$use<0.5,"low","high")
# useabund2$useabund <- useabund2$use*(useabund2$beta+(-1*min(useabund2$beta)))
useabund2$useabund2 <- ifelse(useabund2$use<0.05,NA,useabund2$beta)
useabund2$useabund3 <- ifelse(useabund2$use==0,NA,useabund2$beta)
useabund2$useabund4 <- ifelse(useabund2$use<0.05,NA,exp(useabund2$beta))

base_size <- 12
pdf("~/dat/bipartitemodelsBC/thesis revision/abundheatmap-color.pdf",height=12)
...
### TRY THIS ON A LOG COLOR SCALE TOO?..no
# pdf("~/dat/bipartitemodelsBC/figs/useheatmap-sort2.pdf",height=12)
ggplot(probusesort2, aes(host.sp,par.sp)) + geom_tile(aes(fill = use), colour = "white") + scale_fill_gradient(low = "white", high = "black",trans="log") + theme_grey(base_size = base_size) + labs(x = "", y = "") + scale_x_discrete(expand = c(0, 0)) + scale_y_discrete(expand = c(0, 0)) + theme(legend.position = "none", axis.ticks = element_blank(), axis.text.x = element_text(size = base_size * 0.8, angle = 270, hjust = 0, colour = "grey50"),axis.text.y=element_text(size=base_size*0.7,vjust=0.25))
# 
# dev.off()
...
pdf("~/dat/bipartitemodelsBC/thesis revision/host-pardiv.pdf")
par(oma=c(4, 11, 0, 1), mar=c(0, 0.3, 2, 0), bg ='white') # try changing all margins to 0??
plot.new()
with(ID[1:nrow(ID),], {
  plot.window(xlim = range(c(0,ID$q97.5)), ylim =range(c(1, nrow(ID))))
  box()
  axis(1)
...
pdf("~/dat/bipartitemodelsBC/thesis revision/par-hostbr.pdf",height=11)
...
usemerge$presence <- ifelse(usemerge$prev > 0,1,0)
...
domobs <- obshost[rownames(obshost) %in% doms,colnames(obshost) %in% doms]
wildobs <- obshost[!(rownames(obshost) %in% doms),!(colnames(obshost) %in% doms)]

gd <- graph.adjacency(domobs,mode="undirected",diag=FALSE,weighted=TRUE)
gw <- graph.adjacency(wildobs,mode="undirected",diag=FALSE,weighted=TRUE)
ebcd <- edge.betweenness.community(gd, directed=F)
...
plot.igraph(gd,layout=layout.circle(gd), edge.color="grey",edge.width=E(gd)$weight,main="Observed, Domestic")
plot.igraph(g0,layout=layout.circle(g0), edge.color="grey",edge.width=E(g0)$weight,main="Observed")
plot.igraph(g1,layout=layout.circle(g1), edge.color="grey",edge.width=E(g1)$weight,main="Lower bound")
plot.igraph(gw,layout=layout.circle(gw), edge.color="grey",edge.width=E(gw)$weight,main="Observed, Wild")
plot.igraph(g2,layout=layout.circle(g2), edge.color="grey",edge.width=E(g2)$weight,main="Median")
plot.igraph(g3,layout=layout.circle(g3), edge.color="grey",edge.width=E(g3)$weight,main="Upper bound")
dev.off()
